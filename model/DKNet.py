    ...
    def forward(self, input, input_map, coords, batch_idxs, epoch, ins_sample_num, train=True):
        ...
        if(epoch > self.semantic_epochs):
            ...
            if epoch > (cfg.prepare_epochs + cfg.semantic_epochs):
                ...
                ret['decoding'] = masks, thres, thre_masks, seg_result, ins_sem_preds
...
def train_fn(model, data, epoch):
    ...
    input_ = spconv.SparseConvTensor(voxel_feats, voxel_coords.int(), spatial_shape, coords[:, 0].int().max()+1)
    ret = model(input_, p2v_map, coords_float, coords[:, 0].int(), epoch,  input_.batch_size*150)
    ...
    if(epoch > cfg.semantic_epochs):
        ...
        if epoch > (cfg.prepare_epochs + cfg.semantic_epochs):
            masks, thres, thre_masks, seg_result, ins_sem_preds = ret['decoding']
...
def test_fn(data, model, epoch):
    ...
    with torch.no_grad():
        ...
        if (epoch > cfg.semantic_epochs):
            ...
            preds['candidate'] = (candidate_pt, candidate_score)
            preds['decoding'] = (masks, thres, thre_masks, seg_result, ins_sem_preds)
